...
def save_board_state():
    global move_history, score_history  # Lưu trạng thái hiện tại của bảng
    move_history.append([row[:] for row in board_values])
    score_history.append(score)

# Undo last move
def undo_last_move():
    global board_values, score
...
def take_turn(direc, board):
    ...
    merge_sound.play()  # Phát âm thanh khi gộp ô
    return board
...
def draw_board():
    ...
    screen.blit(reset_icon_resized, (280, 430))
    screen.blit(undo_icon_resized, (340, 430))
...
while run:
    ...
    if spawn_new or init_count < 2:
        ...
        init_count += 1
    if direction != '':
        save_board_state()  # Lưu trạng thái trước khi thực hiện hành động
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if 280 <= mouse_pos[0] <= 280 + 32 and 430 <= mouse_pos[1] <= 430 + 32:
                board_values = [[0 for _ in range(4)] for _ in range(4)]
                spawn_new = True
                init_count = 0
                score = 0
                ...
                move_history = []
                score_history = []
            if 340 <= mouse_pos[0] <= 340 + 32 and 430 <= mouse_pos[1] <= 430 + 32:
